...
class ChamferDistanceL2_return_idx(torch.nn.Module):
    f''' Chamder Distance L2 no mean
        xyz1: [b, n, 3]
        xyz2: [b, n, 3]
        dist1, dist2: [b, n]
        mean: [b]
    ...
    def forward(self, xyz1, xyz2):
        ...
        dist1, dist2 = ChamferFunction.apply(xyz1, xyz2)
        mean = torch.mean(dist1, dim=1) + torch.mean(dist2, dim=1)
        index = torch.min(mean, dim=0)[1]
        return index
